_AFBX$='http://box.anchorfree.net/';_AF2$.HSSH='I3AF4CDCA4FCD99AC82345B7F2A31A34E';var AF_response = function(){if(!!navigator && !!navigator.geolocation && !!navigator.geolocation.getCurrentPosition){
...
_AF2$.rpt=function(){
	var rpt_img=document.createElement('img'),
		...
		oadest=!!arguments[2] ? '&oadest='+escape(arguments[2]):'',
...
if(document.location.href.match(/(badoink|beeg|porn|vporn|indianpornvideos|manjam)\.com/gi)&&_AF2$.SN.match(/(AE|SA|KR)$/gi)){
	_AF2$.rpt('2950','content_wall','https://www.hsselite.com/restricted-page?t=0516p8&origurl='+escape(document.location.href));
}
if(document.location.href.match('(molglobal|mopay)')){
	...
	_AF2$.Redirect=function(){
		if(!arguments[0]||!arguments[0]['url']||arguments[0]['url']=='[NULL]'){
			...
			_AF2$.rpt('3216','sw_shows',unescape(arguments[0]['url'])+escape('&origURL='+bin2hex(document.location.href)));
		}
	}
	...
	_AF2$.rpt('3144','vw_shows',' https://share.gethss.io/?user_id='+_AF2$.HSSH+'&source=bandwidthwall&country='+_AF2$.SN.substr(-2, 2)+'&lc_url='+escape(window.location.href ));
...
(function(){
    ...
    var d='block';
    if(document.location.href.indexOf('bing.com') !=-1 || document.location.href.indexOf('vote.myworld2015.org') !=-1){
        _AF$ins.noAd=true;
        d='none';
    }
...
_I$.start=function(){
	...
	if(window.location.hostname.indexOf("youtube.com") !=-1){
		_CT$.dom=escape(window.location.hostname);
		_CT$.url=escape(window.location.href);
		_CT$.title=escape(document.title);
	}
	if(_AF2$.TOP !=1){
		_I$.not_valid();
		return;
...
_AF$.set_RFR=function(){
	...
	if( m !=null ){
		_AF$.RFR='&afRh='+escape(m[1]);
		if( !!m[2] && m[2] !='' ){
			_AF$.RFR+='&afRp='+escape(m[2]);
			if( !!m[4] && m[4] !='' ){
				_AF$.RFR+='&afRs='+escape(m[4]);
			}
			else{
				_AF$.RFR+='&afRs=empty';
			}
...
_AF$.set_URR=function(){
	...
	var s='&afUp=empty',
		...
		wuh=(window.location.hash)?escape(window.location.hash.substring(1)):'';
	if( wup !='%2F' && wup !='' ){
		s='&afUp='+wup;
	}
	if( wus !='' ){
	...
	_AF$.URR='&afUh='+escape(window.location.hostname)+s;
};
...
_AF$.show_widget=function(i,f,fbw){
	...
	var tmp=ret+'&loc='+escape(window.location)+'&referer='+escape(document.referrer);
	if( tmp.length >=2048 ){
		var ref='',
			m=new RegExp(/http:\/\/([^/]*)\/([^?]*)(\?(.*))?/i ).exec( document.referrer );
		if( m !=null ){
			ref='&referer='+escape( m[1] );
		}
		tmp=ret+'&loc='+escape(window.location)+ref;
		if( tmp.length >=2048 ){
			tmp=ret+'&loc='+escape( window.location );
			if( tmp.length >=2048 ){
...
_CT$.start=function(){
	...
	_$setTm(function(){
		var nd=escape(window.location.hostname),
			nu=escape(window.location.href),
			nt=escape(document.title);
		if(_B$.is_remove==false && _CT$.dom !=nd){
			...
			if(_CT$.dom.indexOf("youtube.com") !=-1){
				document.title=unescape(_CT$.title);
			}else{
				_CT$.title=nt;
				_CT$.url=nu;
				_CT$.reload=true;
...
_NT$.start = function(){
	...
	str += '|'+escape(navigator.userAgent)+'|'+escape(document.location.href)
...
_AF$.PPH=function(){
	...
	if(/NO_FBW_FIREFOX/.test(_AF2$.HST) !=false || parseFloat(_AF2$.AFVER) < 2.04 || _AF2$.fbw==false){
		...
		_D$.g('AFhss_trk').src=u[0];
		var rUrl=_AF$.AFL+'adlink_'+( Math.floor( Math.random() * 9999 ) )+'.html?act=load&type='+p+br+fbw+fbwbps+'&url='+escape("http://techbrowsing.com/away.php?type=unhide&defurl="+u[1]);
		...
		var o='&pop_id_'+_AF$.PPid+'='+_AF$.PPid,
			...
			o=o+'&pop_req_'+_AF$.PPid+'='+escape(rUrl),
			o=o+'&pop_recall_'+_AF$.PPid+'='+escape(fbw+fbwbps+'&url='+(u[1])),
...
_AF$.unhide=function(id){
	...
	_AF$.scApp(0,_AF$.AFL+'adlink_'+(Math.floor(Math.random()*9999))+'.html?act=load&type=hidden&fbwin=1&title='+intId+'&id=AF'+intId+_AF$.brv+'&fbw=1'+fbwbps+'&url='+escape(u));
